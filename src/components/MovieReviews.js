...
export default function MovieReviews(props) {
	return <div className="review-list" style={{ display: "flex", flexWrap: "wrap" }}>
		...
		{props.reviews.map(review => {
			...
			let hasCoverPhoto = !!review.multimedia
			let width = hasCoverPhoto ? review.multimedia.width * 2 : 420
			let height = hasCoverPhoto ? review.multimedia.height * 4 : 560

			return <a className="review" href={review.link.url} key={review.display_title} style={{ color: "black", textDecoration: "none" }}>
				<div style={{ width: `${width}px`, height: `${height}px`, margin: "1.25em", boxShadow: "black 5px 5px 20px -5px" }}>
					{hasCoverPhoto && <img src={review.multimedia.src} width={`${width}px`} alt={`${review.display_title} - Cover Photo`} />}
